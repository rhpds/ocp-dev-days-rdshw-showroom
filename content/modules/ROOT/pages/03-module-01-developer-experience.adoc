...
. At the bottom of the chat panel, click the persona selector and choose **Code** -- since we are editing implementation code, not designing major changes.
+
...
Your changes are now merged into the main branch. To see the dev environment pipeline that picks this up automatically, navigate back to *Developer Hub*, open the *Catalog*, and select the primary *Parasol Insurance* component -- this is the top-level component, _not_ the one associated with your feature branch. The pipeline will redeploy to the dev environment. To promote to production, you need to create a release.
...
Creating this tag triggers a pipeline behind the scenes. You can monitor its progress in *Developer Hub* by navigating to the *Catalog* and selecting the primary *Parasol Insurance* component (not the feature branch one) -- check the *CI* tab to see the tag pipeline running. When the pipeline completes (it will take a few minutes), it generates a merge request against your `{user}/parasol-insurance-manifests` repository -- this is the GitOps manifests repo that drives production deployments through Argo CD.
...
+
TIP: If the merge request has not appeared yet, wait a minute -- the tag pipeline needs to complete first. You can check its progress on the primary *Parasol Insurance* component's *CI* tab in Developer Hub. Refresh this page to check again once the pipeline finishes.
